...
def build_digest(jobs: list, sent_ids: set, config: dict) -> list[str]:
    """Calls Claude with the brief's prompt format. Returns a list of
    separate WhatsApp message strings (one per qualifying job), or an
    empty list if there's nothing new to send."""
    if not jobs:
        return []
    ...
    return [msg.strip() for msg in text.split("===JOB_BREAK===") if msg.strip()]
